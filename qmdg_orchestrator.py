...
class AIOrchestrator:
    ...
    def run_pipeline(self, user_question, current_topic="Chung", chart_data=None, mai_hoa_data=None, luc_hao_data=None):
        ...
        system_prompt = (
            ...
            f"QUY TẮC BẮT BUỘC:\n"
            f"1. BẮT BUỘC căn cứ vào [DỮ LIỆU BÀN CỜ ĐANG HIỂN THỊ] (nếu có) để đưa ra kết luận. KHÔNG TRẢ LỜI CHUNG CHUNG.\n"
            f"2. BẮT BUỘC xác định đúng đối tượng (Dụng Thần) cần xem từ [BẢNG TRA CỨU ĐỐI TƯỢNG].\n"
            f"3. Nếu câu hỏi về con người (ai, trai, gái) mà dữ liệu cung cấp chỉ nói về chủ đề khác (như Bán nhà), hãy BỎ QUA chủ đề đó và chỉ dùng [BẢNG TRA CỨU ĐỐI TƯỢNG] và [TỪ ĐIỂN TÍNH CÁCH] để trả lời.\n"
            f"4. TRÌNH BÀY: \n"
            f"   - Dùng [SUY_LUAN]...[/SUY_LUAN] để giải thích quy trình tư duy.\n"
            f"   - Dùng [KET_LUAN]...[/KET_LUAN] để đưa ra câu trả lời cuối cùng.\n"
            f"5. Tuyệt đối không tự ý bịa đặt thông tin nếu dữ liệu không có.\n"
        )

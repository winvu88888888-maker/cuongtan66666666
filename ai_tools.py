
import datetime

# --- LUNAR CALENDAR TOOLS ---
# Simplified algorithmic converter (Approximate for 1900-2100)
# Ideally we should use a library like 'lunisolar' or 'lunar_python'
# but here we include a lightweight heuristic or rely on simple mapping if libs missing.

def get_khong_minh_luc_dieu(lunar_month, lunar_day):
    """
    Tính Khổng Minh Lục Diệu cho cả ngày (tương ứng 12 khung giờ).
    Công thức: (Tháng + Ngày + ChiGiờ - 2) % 6
    ChiGiờ: Tý=1, Sửu=2, ..., Hợi=12
    """
    luc_dieu_dict = {
        1: "Đại An (Tốt) - Mọi việc yên tâm, hành sự thành công.",
        2: "Lưu Niên (Xấu) - Việc trễ nải, rắc rối giấy tờ.",
        3: "Tốc Hỷ (Tốt) - Tin vui đến nhanh, cầu tài đắc lợi.",
        4: "Xích Khẩu (Xấu) - Cãi vã, thị phi, tranh chấp.",
        5: "Tiểu Cát (Tốt) - May mắn nhỏ, trôi chảy, được giúp đỡ.",
        0: "Không Vong (Xấu) - Mất mát, hư hỏng, việc không thành."
    }
    
    chi_hours = [
        "Tý (23h-1h)", "Sửu (1h-3h)", "Dần (3h-5h)", "Mão (5h-7h)", 
        "Thìn (7h-9h)", "Tỵ (9h-11h)", "Ngọ (11h-13h)", "Mùi (13h-15h)", 
        "Thân (15h-17h)", "Dậu (17h-19h)", "Tuất (19h-21h)", "Hợi (21h-23h)"
    ]
    
    results = []
    good_hours = []
    
    for i, hour_label in enumerate(chi_hours):
        chi_val = i + 1
        val = (lunar_month + lunar_day + chi_val - 2) % 6
        meaning = luc_dieu_dict[val if val != 0 else 0] # 0 is Khong Vong
        
        entry = f"- {hour_label}: {meaning}"
        results.append(entry)
        
        if val in [1, 3, 5]: # Dai An, Toc Hy, Tieu Cat
            good_hours.append(hour_label.split(' ')[0]) # Just the name "Tý"
            
    summary = f"Các giờ Tốt xuất hành hôm nay: {', '.join(good_hours)}"
    details = "\n".join(results)
    
    return summary, details

# --- LUNAR CONVERTER (Offline/Standalone) ---
# Since we don't have internet to pip install, use a basic conversion 
# or try to import library if available. 
# FALLBACK: If we can't convert, request user input or assume Solar~Lunar (bad but runnable)
# Better: Use 'lunardate' or 'lunar_python' if present. 

# --- STANDALONE LUNAR CALENDAR (NO DEPENDENCIES) ---
# Source: Optimized Hex Table for 1900-2100 (Common Alogrithm)

class MiniLunarCalendar:
    # Hex data for years 1900-2100 (compressed)
    # Each hex contains info about leap month, big/small months
    LUNAR_DATA = [
        0x04bd8, 0x04ae0, 0x0a570, 0x054d5, 0x0d260, 0x0d950, 0x16554, 0x056a0, 0x09ad0, 0x055d2,
        0x04ae0, 0x0a5b6, 0x0a4d0, 0x0d250, 0x1d255, 0x0b540, 0x0d6a0, 0x0ada2, 0x095b0, 0x14977,
        0x04970, 0x0a4b0, 0x0b4b5, 0x06a50, 0x06d40, 0x1ab54, 0x02b60, 0x09570, 0x052f2, 0x04970,
        0x06566, 0x0d4a0, 0x0ea50, 0x06e95, 0x05ad0, 0x02b60, 0x186e3, 0x092e0, 0x1c8d7, 0x0c950,
        0x0d4a0, 0x1d8a6, 0x0b550, 0x056a0, 0x1a5b4, 0x025d0, 0x092d0, 0x0d2b2, 0x0a950, 0x0b557,
        0x06ca0, 0x0b550, 0x15355, 0x04da0, 0x0a5d0, 0x14573, 0x052d0, 0x0a9a8, 0x0e950, 0x06aa0,
        0x0aea6, 0x0ab50, 0x04b60, 0x0aae4, 0x0a570, 0x05260, 0x0f263, 0x0d950, 0x05b57, 0x056a0,
        0x096d0, 0x04dd5, 0x04ad0, 0x0a4d0, 0x0d4d4, 0x0d250, 0x0d558, 0x0b540, 0x0b5a0, 0x195a6,
        0x095b0, 0x049b0, 0x0a974, 0x0a4b0, 0x0b27a, 0x06a50, 0x06d40, 0x0af46, 0x0ab60, 0x09570,
        0x04af5, 0x04970, 0x064b0, 0x074a3, 0x0ea50, 0x06b58, 0x055c0, 0x0ab60, 0x096d5, 0x092e0,
        0x0c960, 0x0d954, 0x0d4a0, 0x0da50, 0x07552, 0x056a0, 0x0abb7, 0x025d0, 0x092d0, 0x0cab5,
        0x0a950, 0x0b4a0, 0x0baa4, 0x0ad50, 0x055d9, 0x04ba0, 0x0a5b0, 0x15176, 0x052b0, 0x0a930,
        0x07954, 0x06aa0, 0x0ad50, 0x05b52, 0x04b60, 0x0a6e6, 0x0a4e0, 0x0d260, 0x0ea65, 0x0d530,
        0x05aa0, 0x076a3, 0x096d0, 0x04bd7, 0x04ad0, 0x0a4d0, 0x1d0b6, 0x0d250, 0x0d520, 0x0dd45,
        0x0b5a0, 0x056d0, 0x055b2, 0x049b0, 0x0a577, 0x0a4b0, 0x0aa50, 0x1b255, 0x06d20, 0x0ada0
    ]
    
    BASE_YEAR = 1900
    BASE_DATE = datetime.date(1900, 1, 31)

    @classmethod
    def solar_to_lunar(cls, d):
        days_in_year = 365
        offset = (d - cls.BASE_DATE).days
        
        if offset < 0: return d.month, d.day # Fallback pre-1900

        for year in range(cls.BASE_YEAR, 2101):
            days_in_year = cls._days_in_lunar_year(year)
            if offset < days_in_year:
                break
            offset -= days_in_year
            
        lunar_year = year
        leap_month = cls._get_leap_month(year)
        is_leap = False
        
        lunar_month = 1
        for i in range(1, 13):
            days_in_month = cls._days_in_lunar_month(year, i)
            if offset < days_in_month:
                lunar_month = i
                break
            offset -= days_in_month
            
            if leap_month == i:
                days_in_leap = cls._days_in_leap_month(year)
                if offset < days_in_leap:
                    is_leap = True
                    lunar_month = i
                    break
                offset -= days_in_leap
                
        lunar_day = offset + 1
        return lunar_month, lunar_day, is_leap

    @classmethod
    def _days_in_lunar_year(cls, year):
        days = 348
        for i in range(0x8000, 0x8, -1):
            days += 1 if (cls.LUNAR_DATA[year - cls.BASE_YEAR] & i) else 0
        days += cls._days_in_leap_month(year) if cls._get_leap_month(year) else 0
        return days

    @classmethod
    def _days_in_lunar_month(cls, year, month):
        return 29 if not (cls.LUNAR_DATA[year - cls.BASE_YEAR] & (0x10000 >> month)) else 30

    @classmethod
    def _get_leap_month(cls, year):
        return cls.LUNAR_DATA[year - cls.BASE_YEAR] & 0xf

    @classmethod
    def _days_in_leap_month(cls, year):
        return 0 if not cls._get_leap_month(year) else (30 if (cls.LUNAR_DATA[year - cls.BASE_YEAR] & 0x10000) else 29)

def get_lunar_date_offline(d):
    """
    Returns (month, day) using built-in algorithm.
    No 3rd party library required.
    """
    try:
        m, d, _ = MiniLunarCalendar.solar_to_lunar(d)
        return m, d
    except Exception as e:
        print(f"Lunar Calc Error: {e}")
        return d.month, d.day

